...
class SigmoidFocalClassificationLoss(nn.Module):
    ...
    def reweight(self, cls_num_list, beta=0.9999):
        """
        ...
        per_cls_weights = torch.tensor(per_cls_weights).type(torch.FloatTensor).cuda()
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        #def forward(self, input, target, weights):
            ...
            #############################################################################
            cls_num_list = [14357,2207,734]
            weights = self.reweight(cls_num_list, 0.9999)
            m = nn.LogSoftmax(dim=1)
            BCE_loss = F.binary_cross_entropy_with_logits(input, target, weight=weights, reduction='none')
            pt = torch.exp(-BCE_loss)
            f_loss =  ((1.0 - pt) ** self.gamma) * BCE_loss
            loss = f_loss.mean()
   #         loss_func = nn.NLLLoss(weight=weights)
   #         input=torch.einsum('ijk->ikj', input)
   #         target=torch.einsum('ijk->ij', target).long()
            #log_prob = m(input)
            #prob = torch.exp(log_prob)
            #focal_term = ((1.0 - prob) ** self.gamma) * log_prob
            #print('n1', focal_term.shape)
            #print('n2', weights)
            #print('n3', target.shape)
            #loss = loss_func(focal_term, target)
